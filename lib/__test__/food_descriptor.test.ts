...
class GenieRestaurant extends DataClass {
  ...
  static all(): GenieRestaurant[] {
    ...
    return state.restaurant.restaurants.map((r) =>
      GenieRestaurant.fromSanityRestaurant(r)
    );
  }

  static current(): GenieRestaurant {
    return GenieRestaurant.fromSanityRestaurant(
      state.restaurant.restaurants[state.restaurant.activeRestaurantIdx]
    );
  }
  ...
  static select(restaurant: GenieRestaurant) {
    const idx = state.restaurant.restaurants.findIndex(
      (r) => r._id === restaurant.id
    );
    if (idx !== -1) {
...
class GenieOrder extends DataClass {
  static addFoods({ dishes }: { dishes: GenieDish[] }) {
    ...
    state.basket.items = [
      ...state.basket.items,
      ...findAllDishes().filter((dish) => ids_to_add.includes(dish._id)),
    ];
  }
  ...
  static removeFoods({ dishes }: { dishes: GenieDish[] }) {
    ...
    state.basket.items = state.basket.items.filter(
      (item) => !ids_to_remove.includes(item._id)
    );
  }
