...
export class BasicPromptGen {
  constructor(private class_descriptions: string[], private examples: ExampleParse[], private extraPrompt?: string) {
  }

  prompt_basic(): string {
    let prompt = `${class_prequel}${pre_section_separator}` +
      `${array_definition}${class_separator}` +
      `${this.class_descriptions.join(class_separator)}${post_section_separator}` +
...
export class DescriptorPromptGen extends BasicPromptGen{
  constructor(private class_descriptors: ClassDescriptor<GenieObject>[], examples: ExampleParse[], extraPrompt?: string) {
