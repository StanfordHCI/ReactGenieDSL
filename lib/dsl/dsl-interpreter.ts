...
import {ClassDescriptor, FuncDescriptor, GenieObject, ParamDescriptor} from "../dsl-descriptor";

function parseType(value_type: string): { is_array: boolean, original_type: string } {
    if (value_type.endsWith("[]")) {
        return {is_array: true, original_type: value_type.substring(0, value_type.length - 2)};
    } else {
...
export class DslInterpreter {
    ...
    private matching(value: any, template: any) {
        ...
        // primitive types
            ...
            return JSON.stringify(value._getConstructorParams()) === JSON.stringify(template._getConstructorParams());
        }
    }
    ...
    private arrayFunctionDescriptors: FuncDescriptor[] = [
        new FuncDescriptor("matching", [new ParamDescriptor("field", "accessor"), new ParamDescriptor("value", "object")], "object[]", false),
        new FuncDescriptor("contains", [new ParamDescriptor("field", "accessor"), new ParamDescriptor("value", "object")], "object[]", false),
        new FuncDescriptor("between", [new ParamDescriptor("field", "accessor"), new ParamDescriptor("from", "object"), new ParamDescriptor("to", "object")], "object[]", false),
        new FuncDescriptor("equals", [new ParamDescriptor("field", "accessor"), new ParamDescriptor("value", "object")], "object[]", false),
        new FuncDescriptor("sort", [new ParamDescriptor("field", "accessor"), new ParamDescriptor("ascending", "boolean")], "object[]", false),
        new FuncDescriptor("index", [new ParamDescriptor("index", "int")], "object", false),
    ]
    // implementations of functions
    private arrayFunctionImplementations: { [name: string]: (...args: any[]) => any } = {
        matching: ({field, value, array}: { field: any, value: any, array: any[] }) => {
            console.assert(field.type === "accessor");
            const fieldName = field.field;
            return array.filter(v => this.matching(v[fieldName], value));
        },
        contains: ({field, value, array}: { field: any, value: any, array: any[] }) => {
            console.assert(field.type === "accessor");
            const fieldName = field.field;
            return array.filter(v => this.contains(v[fieldName], value));
        },
        between: ({field, from, to, array}: { field: any, from: any, to: any, array: any[] }) => {
            console.assert(field.type === "accessor");
            const fieldName = field.field;
            return array.filter(v => {
                ...
                let fromIsPrimitive = typeof from === "string" || typeof from === "number" || typeof from === "boolean";
                let toIsPrimitive = typeof to === "string" || typeof to === "number" || typeof to === "boolean";
                let valueIsPrimitive = typeof value === "string" || typeof value === "number" || typeof value === "boolean";
        ...
        equals: ({field, value, array}: { field: any, value: any, array: any[] }) => {
            console.assert(field.type === "accessor");
            const fieldName = field.field;
            return array.filter(v => this.equals(v[fieldName], value));
        },
        sort: ({field, ascending, array}: { field: any, ascending: boolean, array: any[] }) => {
            console.assert(field.type === "accessor");
            const fieldName = field.field;
    ...
    public describe(ast: any) {
        switch (ast.type) {
            case "array":
                return {type: "array", elements: ast.value.map(e => this.describe(e))};
            case "string":
            case "int":
            case "boolean":
                return ast.value;
            case "object":
                if (ast.value !== undefined) {
                    if (ast.objectType == "string") {
                        return {type: "object", objectType: ast.objectType, value: ast.value};
                    } else {
    ...
    public strip(ast: any) {
        switch (ast.type) {
            case "class":
                return this.classDescriptors.find(c => c.className === ast.value).classConstructor;
            case "array":
    ...
    private resolveAccess(ast: any, env: any) {
        ...
        const parent = typeof ast.parent === "string" ?
    ...
    private resolveIndex(ast: any, env: any) {
        ...
        return this.resolve({
            type: "access",
            parent: ast.parent,
            access: {type: "function_call", "func_name": "index", parameters: [{parameter: "index", value: indexParam}]}
        }, env);
    }
    ...
    private resolveFunctionCall(ast: any, env: any) {
        const parameters = ast.parameters !== null ?
            new Map(ast.parameters.map(p => [p.parameter, this.resolve(p.value, null)])) :
            new Map([]);
        ...
        if (env !== null) {
            switch (env.type) {
                ...
                case "array":
                    funcDescriptor =
                        this.arrayFunctionDescriptors.find(f => f.func_name === ast.func_name);
        ...
        const targetImplementation = isArray ?
            this.arrayFunctionImplementations : this.strip(env);

        // call the function
        if (returnType.is_array) {
            return {
                type: "array",
                value: targetImplementation[ast.func_name](matchedParameters).map(v => {
                    return {
                        type: "object",
                        value: v,
