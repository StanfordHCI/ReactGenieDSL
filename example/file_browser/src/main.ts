...
async function main() {
  while (true) {
    ...
    if (lastResult.type === "object") {
      if (
        lastResult.objectType === "string" ||
        lastResult.objectType === "int" ||
        lastResult.objectType === "boolean"
      ) {
      ...
      resultStr = JSON.stringify(
        lastResult.value.map((element) => element.value.description())
      );
    }
    // respond to the user
