...
export class GeniePath extends DataClass {
  ...
  constructor({ absoluteString }: { absoluteString: string }) {
    ...
    if (Path.isAbsolute(absoluteString)) {
      ...
      absolutePath = Path.normalize(
        path.resolve(GeniePath.currentPath.absoluteString, absoluteString)
      );
    }
  ...
  public setup() {
    GeniePath.currentPath = GeniePath.CreateObject({
      absoluteString: os.homedir(),
    });
  }
  ...
  public relativeString(): string {
    return path.relative(
      this.absoluteString,
      GeniePath.currentPath.absoluteString
    );
  }
  ...
  public listDirectory(): GeniePath[] {
    return fs
      .readdirSync(this.absoluteString)
      .map((name) => GeniePath.CreateObject({ absoluteString: name }));
  }
  ...
  public static GetPath({ pathStr = "" }: { pathStr?: string }): GeniePath {
    return GeniePath.CreateObject({
      absoluteString: Path.normalize(
        path.resolve(GeniePath.currentPath.absoluteString, pathStr)
      ),
    });
  }
...
export const GeniePathExample: ExampleParse[] = [
  ...
  },
  {
    ...
    example_parsed:
      "GeniePath.currentPath.listDirectory()[0].changeDirectory()",
  },
  {
